...
if [ $# -eq 0 ] || [ $# -eq 1 ] || [ $1 == "--help" ]; then
	...
	# echo -e "    [output_file] default is out.html"
	...
	#scrapy crawl google -o out.json 2>&1
	 ...
	 scrapy crawl $2 -o ../results/oauth_sites.json 2>&1 

	echo "generating table of unique URLS to try!"
	cd ../results
	python -c"import makeHtml; makeHtml.generate_html('$3')"
	#> final_check_these_sites.txt
	#python -c'import parse_urls; parse_urls.generate_unique_url_list("oauth_sites.json")'
	echo "done!"
fi
